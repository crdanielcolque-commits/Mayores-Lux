...
def severidad_y_motivo(row):
    ...
    var_pct_raw = row.get("Variación %", row.get("Evolución %", np.nan))
...
def tipo_evento(row):
    ...
    var = row.get("Variación $", row.get("Evolución $", 0))
    var_pct = row.get("Variación %", row.get("Evolución %", np.nan))
    if ant == 0 and act > 0:
...
with tab1:
    ...
    centros_opciones = list(dict.fromkeys([c for c in CENTROS_POSVENTA + CENTROS_COSTO if f"{c}_Gestion" in df_filtrado.columns]))

    with c1:
        centro = st.selectbox("Centro de costo / sector", centros_opciones, index=0)
    with c2:
        grupos_sel = st.multiselect("Categoría de costos", ["Costos Controlables", "Costos No Controlables"], default=["Costos Controlables", "Costos No Controlables"])
    with c3:
    ...
    if base.empty:
        ...
        fig = px.line(evo, x="Mes", y="Importe_CC", color="Grupo_PNL", markers=True, text="Etiqueta", title=f"Evolución mensual de costos - {centro}")
        ...
        st.dataframe(
            mostrar[["Grupo_PNL", "Concepto_PNL", "Mes anterior", "Mes actual", "Variación $", "Variación %", "Semáforo", "Severidad", "Motivo severidad", "Tipo evento"]],
        ...
        if not aumentos.empty:
            a = aumentos.iloc[0]
            texto.append(f"Principal presión: **{a['Concepto_PNL']}**, con suba de **{fmt_money(a['Variación $'])}**. Motivo de severidad: {a['Motivo severidad']}.")
        if not bajas.empty:
            b = bajas.iloc[0]
            texto.append(f"Principal mejora: **{b['Concepto_PNL']}**, con baja de **{fmt_money(abs(b['Variación $']))}**. Conviene validar si la baja es sostenible o si corresponde a un diferimiento de facturación.")
        st.info(" ".join(texto))
...
with tab2:
    ...
    if base_a.empty:
        ...
        if alertas.empty:
            ...
            mostrar["Variación %"] = mostrar["Variación %"].apply(fmt_pct)
            st.dataframe(
                mostrar[["Severidad", "Motivo severidad", "Tipo evento", "Grupo_PNL", "Concepto_PNL", "Mes anterior", "Mes actual", "Variación $", "Variación %", "Semáforo"]],
                use_container_width=True,
                height=360
            )
        ...
        for _, r in aumentos.iterrows():
            ...
            top_prov = prov_concepto.groupby("Proveedor_detectado", as_index=False)["Importe_CC"].sum().sort_values("Importe_CC", ascending=False).head(1)
            proveedor_txt = ""
            ...
            insights.append(
                ...
                f"**Hipótesis:** gasto extraordinario, ajuste de contrato o concentración puntual de facturación."
                f"{proveedor_txt} **Acción sugerida:** revisar movimientos, proveedor principal y recurrencia."
            )

        for _, r in mejoras.iterrows():
            insights.append(
                f"🟢 **{r['Concepto_PNL']}** mejora **{fmt_money(abs(r['Variación $']))}**. "
                f"**Lectura:** puede ser eficiencia real, diferimiento de facturación o cambio temporal. "
                f"**Acción sugerida:** validar si la mejora es sostenible."
            )
        ...
        if not prov.empty:
            ...
            if len(meses) >= 2:
                ...
                if not pv.empty and pv.iloc[0]["Variación"] > 0:
                    insights.append(
                        f"🏭 **Proveedor con mayor presión:** **{pv.iloc[0]['Proveedor_detectado']}**, sube **{fmt_money(pv.iloc[0]['Variación'])}**. "
...
with tab3:
    ...
    if base_p.empty:
        ...
        fig = px.bar(stacked, x="Proveedor_detectado", y="Importe_CC", color="Mes", text="Etiqueta", title="Top proveedores por gasto, dividido por mes")
        fig.update_layout(xaxis_tickangle=-45)
        fig.update_traces(textposition="inside", insidetextanchor="middle")
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("### Tabla mensual por proveedor")
        tabla_prov = prov_mes.pivot_table(index=["Proveedor_detectado", "Categoria_proveedor"], columns="Mes", values="Importe_CC", aggfunc="sum", fill_value=0).reset_index()
        ...
        tabla_prov["% concentración pico"] = np.where(tabla_prov["Total"].abs() > 0, tabla_prov["Importe mes pico"] / tabla_prov["Total"].abs(), 0)
        ...
        st.dataframe(tabla_mostrar[["Proveedor_detectado", "Categoria_proveedor"] + meses_cols + ["Total", "Promedio mensual", "Mes pico", "Importe mes pico", "% concentración pico"]].head(50), use_container_width=True, height=430)
        ...
        st.dataframe(crec_m[["Proveedor_detectado", "Categoria_proveedor", "Mes anterior", "Mes actual", "Variación $", "Variación %", "Severidad", "Motivo severidad", "Tipo evento"]], use_container_width=True, height=420)
        ...
        fig2 = px.line(evo, x="Mes", y="Acumulado", color="Proveedor_detectado", markers=True, text="Etiqueta", title="Evolución acumulada por proveedor")
        ...
        proveedores_filtrados = [p for p in proveedores_lista if busqueda.upper() in p.upper()] if busqueda else proveedores_lista
...
with tab4:
    ...
    if base_b.empty:
        ...
        fig = px.bar(total_suc, x="Sucursal", y="Costo total", text="Etiqueta", title=f"Costo total por sucursal - {centro_b}")
        ...
        heat_num = bench.pivot_table(index="Concepto_PNL", columns="Sucursal", values="Importe_CC", aggfunc="sum", fill_value=0)
        ...
        if not total_suc.empty:
            ...
            concepto_mayor_suc = base_b[base_b["Sucursal"] == mayor["Sucursal"]].groupby("Concepto_PNL", as_index=False)["Importe_CC"].sum().sort_values("Importe_CC", ascending=False).head(3)
            conceptos_txt = ", ".join([f"{r['Concepto_PNL']} ({fmt_money(r['Importe_CC'])})" for _, r in concepto_mayor_suc.iterrows()])
            st.info(
                f"🧠 **{mayor['Sucursal']}** concentra **{fmt_pct(part_mayor)}** del costo total analizado en **{centro_b}**, con **{fmt_money(mayor['Costo total'])}**. "
...
with tab6:
    ...
    centros_cta = [c for c in CENTROS_COSTO if f"{c}_Gestion" in df_filtrado.columns]

    with col1:
        centro_cta = st.selectbox("Centro de costo", centros_cta, index=centros_cta.index("Repuestos") if "Repuestos" in centros_cta else 0, key="centro_cuentas")
    with col2:
        grupos_cta = st.multiselect("Categoría", ["Costos Controlables", "Costos No Controlables"], default=["Costos Controlables", "Costos No Controlables"], key="grupo_cuentas")
    with col3:
        meses_cta = st.multiselect("Meses", meses_all, default=meses_all[-4:], key="meses_cuentas")

    campo_cta = f"{centro_cta}_Gestion"
    base_cta = df_filtrado[(df_filtrado["Grupo_PNL"].isin(grupos_cta)) & (df_filtrado["Mes"].isin(meses_cta))].copy()
    base_cta["Importe_CC"] = base_cta[campo_cta]
    base_cta = base_cta[base_cta["Importe_CC"] != 0].copy()

    if base_cta.empty:
        st.info("No hay datos para los filtros seleccionados.")
    else:
        meses_ordenados = sorted(base_cta["Mes"].dropna().unique())

        tabla_cta = base_cta.pivot_table(
            index=["Cuenta:", "Nombre cuenta", "Rubro_calc", "Nombre del rubro", "Grupo_PNL"],
            columns="Mes",
            values="Importe_CC",
            aggfunc="sum",
            fill_value=0
        ...
        for m in meses_ordenados:
            if m not in tabla_cta.columns:
                tabla_cta[m] = 0

        if len(meses_ordenados) >= 2:
            mes_ant = meses_ordenados[-2]
            mes_act = meses_ordenados[-1]
            tabla_cta["Mes anterior"] = tabla_cta[mes_ant]
            tabla_cta["Mes actual"] = tabla_cta[mes_act]
            tabla_cta["Evolución $"] = tabla_cta["Mes actual"] - tabla_cta["Mes anterior"]
            tabla_cta["Evolución %"] = np.where(tabla_cta["Mes anterior"].abs() > 0, tabla_cta["Evolución $"] / tabla_cta["Mes anterior"].abs(), np.nan)
        else:
            tabla_cta["Mes anterior"] = np.nan
            tabla_cta["Mes actual"] = tabla_cta[meses_ordenados[-1]]
            tabla_cta["Evolución $"] = np.nan
            tabla_cta["Evolución %"] = np.nan

        tabla_cta["Impacto_abs"] = tabla_cta["Evolución $"].abs()
        tabla_cta["Semáforo"] = tabla_cta["Evolución $"].apply(semaforo_variacion)
        sev_motivos = tabla_cta.apply(severidad_y_motivo, axis=1)
        tabla_cta["Severidad"] = [x[0] for x in sev_motivos]
        tabla_cta["Motivo severidad"] = [x[1] for x in sev_motivos]
        tabla_cta["Tipo evento"] = tabla_cta.apply(tipo_evento, axis=1)
        tabla_cta = tabla_cta.sort_values("Impacto_abs", ascending=False)

        total_actual = tabla_cta["Mes actual"].sum()
        total_anterior = tabla_cta["Mes anterior"].sum()
        evolucion_total = total_actual - total_anterior
        evolucion_pct = evolucion_total / abs(total_anterior) if total_anterior != 0 else np.nan

        k1, k2, k3, k4 = st.columns(4)
        k1.metric("Centro de costo", centro_cta)
        k2.metric("Costo mes actual", fmt_money(total_actual))
        k3.metric("Evolución total", fmt_money(evolucion_total), fmt_pct(evolucion_pct))
        k4.metric("Cuentas analizadas", len(tabla_cta))

        st.markdown("### Top cuentas con mayor evolución vs mes anterior")
        top_cta = tabla_cta.dropna(subset=["Evolución $"]).head(15).copy()
        top_cta["Etiqueta"] = top_cta["Evolución $"].apply(fmt_money_short)
        ...
        if not top_cta.empty:
            fig = px.bar(
                ...
                text="Etiqueta",
                orientation="h",
                title="Cuentas con mayor variación absoluta vs mes anterior"
            )
            preparar_para_labels(fig)
            st.plotly_chart(fig, use_container_width=True)

        st.markdown("### Evolución mensual por cuenta contable")
        mostrar_cta = tabla_cta.copy()
        for m in meses_ordenados:
            mostrar_cta[m] = mostrar_cta[m].apply(fmt_money)
        for col in ["Mes anterior", "Mes actual", "Evolución $"]:
            mostrar_cta[col] = mostrar_cta[col].apply(fmt_money)
        mostrar_cta["Evolución %"] = mostrar_cta["Evolución %"].apply(fmt_pct)

        columnas_mostrar = [
            "Cuenta:", "Nombre cuenta", "Rubro_calc", "Nombre del rubro", "Grupo_PNL"
        ] + meses_ordenados + [
            "Evolución $", "Evolución %", "Semáforo", "Severidad", "Motivo severidad", "Tipo evento"
        ]

        st.dataframe(mostrar_cta[columnas_mostrar], use_container_width=True, height=560)

        st.markdown("### Insight automático por cuenta contable")
        aumentos = tabla_cta[tabla_cta["Evolución $"] > 0].sort_values("Evolución $", ascending=False)
        mejoras = tabla_cta[tabla_cta["Evolución $"] < 0].sort_values("Evolución $", ascending=True)
        texto = []

        if not aumentos.empty:
            a = aumentos.iloc[0]
            texto.append(
                f"🔴 La cuenta con mayor presión es **{a['Cuenta:']} - {a['Nombre cuenta']}**, asociada a **{a['Nombre del rubro']}**, "
                f"con aumento de **{fmt_money(a['Evolución $'])}**. Motivo de severidad: {a['Motivo severidad']}."
            )
        if not mejoras.empty:
            m = mejoras.iloc[0]
            texto.append(
                f"🟢 La principal mejora se observa en **{m['Cuenta:']} - {m['Nombre cuenta']}**, asociada a **{m['Nombre del rubro']}**, "
                f"con baja de **{fmt_money(abs(m['Evolución $']))}**. Conviene validar si es ahorro real o diferimiento de facturación."
            )

        if texto:
            st.info(" ".join(texto))
        else:
            st.success("No se detectan variaciones relevantes por cuenta contable.")

        st.markdown("### Movimientos de la cuenta seleccionada")
        cuentas_lista = (tabla_cta["Cuenta:"].astype(str) + " - " + tabla_cta["Nombre cuenta"].astype(str)).tolist()
        cuenta_sel = st.selectbox("Seleccioná una cuenta para ver movimientos", cuentas_lista, key="cuenta_movimientos")
        cuenta_num = cuenta_sel.split(" - ")[0]

        mov_cta = base_cta[base_cta["Cuenta:"].astype(str) == cuenta_num].copy()
        mov_cta["Importe"] = mov_cta["Importe_CC"].apply(fmt_money)
        mov_cta["Impacto_abs"] = mov_cta["Importe_CC"].abs()
        mov_cta = mov_cta.sort_values("Impacto_abs", ascending=False)

        cols_mov_cta = [
            "Mes", "Fecha", "Sucursal", "Cuenta:", "Nombre cuenta", "Rubro_calc", "Nombre del rubro",
            "Grupo_PNL", "Concepto_PNL", "Proveedor_detectado", "Factura_detectada",
            "Detalle", "des res", "Detalle 2", "Importe"
        ]
        cols_mov_cta = [c for c in cols_mov_cta if c in mov_cta.columns]
        st.dataframe(mov_cta[cols_mov_cta].head(150), use_container_width=True, height=520)

        st.download_button(
            "⬇️ Descargar evolución por cuenta contable",
            data=tabla_cta.to_csv(index=False).encode("utf-8-sig"),
            file_name=f"evolucion_cuentas_{centro_cta}.csv",
            mime="text/csv"
        )
...
with tab8:
    ...
    if not drill.empty:
        rubro_principal = drill.groupby("Nombre del rubro")["Parcial_Gestion"].sum().abs().sort_values(ascending=False).index[0]
        proveedor_principal = drill.groupby("Proveedor_detectado")["Parcial_Gestion"].sum().abs().sort_values(ascending=False).index[0]
        sucursal_principal = drill.groupby("Sucursal")["Parcial_Gestion"].sum().abs().sort_values(ascending=False).index[0]
...
Explica por qué una línea fue clasificada como crítica, alta, media o baja.
...
El tab **📒 Cuentas contables** permite ver:
- número de cuenta
- nombre de cuenta
- rubro + subrubro
- nombre del rubro
